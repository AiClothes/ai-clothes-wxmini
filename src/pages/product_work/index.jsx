...
function Product() {
    ...
    const loadData2 = async (id) => {
        ...
        if (res) {
            ...
            setSwiperList(res.images.map((item) => item.url));
        }
    }
    ...
    useEffect(() => {
        ...
        (async () => {
            setLoadingData(true);
            await Promise.all([
                loadData(Number(router.params.product_id)),
                loadData2(Number(router.params.work_id))
            ]);
                ...
                <View className="product-to-create-wrapper">
                    <View className="product-to-create">
                        <Space style={{width: `100%`, justifyContent: `space-between`}}>
                            <Button size="xlarge" shape="square" type="primary" color="#5596D2" block={true} onClick={() => {
                                // onChangeSelectTab(1);
                                // navigateBack().then((r) => {
                                //     console.log(`已返回路由`, r);
                                // });
                                navigateTo({
                                    url: `/pages/set_type/index?id=${Number(router.params.product_id)}&work_id=${Number(router.params.work_id)}`
                                }).then((r) => {
                                    console.log(`已跳转排版`, r);
                                })
                            }}>
